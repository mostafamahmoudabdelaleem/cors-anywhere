...
function parseURL(req_url) {
  console.log('URL to Parse ==> ' + req_url);
  if(req_url.includes('https')){
    req_url = req_url.replace('https:/', 'http://')
  }
  else if(req_url.includes('http')){
  ...
  if (!match[1]) {
    if (/^https?:/i.test(req_url)) {
      ...
      console.log('Couldn\'t parse ==> ' + req_url);
      return null;
    }
    // Scheme is omitted.
  ...
  console.log('Parsed URL ==> ' + req_url);
  if (!parsed.hostname) {
